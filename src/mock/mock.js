...
axios.interceptors.request.use(
  config => {
    ...
    // 后台 CRUD
    if (url.includes('/yunnanGdp') || url.includes('/cityGdp') || url.includes('/nationalGdp') ||
        url.includes('/quarterGdp') || url.includes('/consumption') || url.includes('/population') ||
        url.includes('/rate') || url.includes('/nationalPopulation') || url.includes('/life')) {
      config.adapter = () => Promise.resolve({
        data: ok(url.includes('find') ? [] : {}),
        status: 200, statusText: 'OK', headers: {}, config, request: {}
      })
    }
